...
public class Node implements Serializable{
    ...
    public void insertOrUpdate(Comparable key, Object obj, BplusTree tree) throws IOException {
        //如果是叶子节点
        if (isLeaf){
            //不需要分裂，直接插入或更新
    ...
    /** 插入到当前节点的关键字中*/
    protected void insertOrUpdate(Comparable key, Object obj) throws IOException {
        RandomAccessFile file=new RandomAccessFile("D:\\Internet\\MyProject\\Bolt\\1.txt","rwd");
        long point=file.length();
        System.out.println("insertpoint:"+point);
        file.seek(point);
        byte[] b=new byte[20];
        byte[] temp=obj.toString().getBytes();
        ...
        Entry<Comparable, Object> entry = new SimpleEntry<Comparable, Object>(key,point);
        //如果关键字列表长度为0，则直接插入
        if (entries.size() == 0) {
            entries.add(entry);
            return;
